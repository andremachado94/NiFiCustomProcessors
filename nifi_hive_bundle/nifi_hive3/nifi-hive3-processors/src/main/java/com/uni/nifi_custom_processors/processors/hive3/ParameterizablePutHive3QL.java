 ...
 */
package com.uni.nifi_custom_processors.processors.hive3;
...
import org.apache.nifi.processors.hive.AbstractHive3QLProcessor;
import org.apache.nifi.processors.hive.PutHive3QL;
import org.apache.nifi.processors.hive.SelectHive3QL;
...
public class ParameterizablePutHive3QL extends AbstractHive3QLProcessor {

    public static final String ORIGIN_ATTRIBUTE = "flowfile-attribute";
    public static final String ORIGIN_CONTENT = "flowfile-content";

    public static final PropertyDescriptor ORIGIN = new PropertyDescriptor.Builder()
            .name("query-origin")
            .displayName("Query Origin")
            .description("Indicates where the query to execute is stored. It can be either in the FlowFile content or attribute;"
                    + " If using attribute, must specify the Attribute Name property")
            .required(true)
            .allowableValues(ORIGIN_CONTENT, ORIGIN_ATTRIBUTE)
            .defaultValue(ORIGIN_ATTRIBUTE)
            .build();
    ...
    public static final PropertyDescriptor QUERY = new PropertyDescriptor.Builder()
            ...
            .displayName("Query")
            .description("The query to execute when Query Origin is " + ORIGIN_ATTRIBUTE)
            .required(false)
            .defaultValue("")
            .build();
    ...
    private Put.PutFlowFile<FunctionContext, Connection> putFlowFile = (context, session, fc, conn, flowFile, result) -> {
        ...
        final String origin = context.getProperty(ORIGIN).getValue();

        if (ORIGIN_ATTRIBUTE.equals(origin)) {
            final String queries = context.getProperty(QUERY).getValue();
            hiveQLs = queries.split(regex);
        }
        else{
            final String script = getHiveQL(session, flowFile, fc.charset);
            hiveQLs = script.split(regex);
        }


        final Set<TableName> tableNames = new HashSet<>();
        String[] finalHiveQLs = hiveQLs;
        exceptionHandler.execute(fc, flowFile, input -> {
            int loc = 1;
            for (String hiveQLStr: finalHiveQLs) {
    ...
    protected Collection<ValidationResult> customValidate(final ValidationContext context) {
        ...
        if (ORIGIN_ATTRIBUTE.equals(origin)) {
            ...
            if (StringUtils.isEmpty(query)) {
                results.add(new ValidationResult.Builder().subject("Query").valid(false)
                        .explanation("The attribute Query must be defined when using " + ORIGIN_ATTRIBUTE).build());
            }
        }
